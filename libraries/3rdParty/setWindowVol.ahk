...
setWindowVol(winName:="a",vol:="n"){
	if (vol=="n")
		return
	/* 
		if an EXE is provided, use the Process command to get the PID. Improves compatibility with Spotify.
	*/
	if (InStr(winName, ".exe")) 
	{
		Process, exist, %winName%
		winPid:=errorlevel
	}
	Else ; use Masonjar13's original PID-retrieval method
	{
